...
<body>
  <main class="layout">
    ...
    <section class="panel comparison-panel">
      ...
      <div class="database-grid">
        <div class="field">
          ...
          <input id="map-name" type="text" value="Generated Manhattan Map" />
        </div>
        <div class="field">
          <label for="map-slug">Map slug</label>
          <input id="map-slug" type="text" value="generated-manhattan-map" />
        </div>
        <div class="field">
          <label for="map-description">Description</label>
          <textarea id="map-description">Generated from the traversal demo grid.</textarea>
        </div>
        <div class="field">
          <label class="checkbox-chip">
            <input type="checkbox" id="save-results-toggle" />
      ...
      <div class="actions">
        ...
        <button id="load-saved-graphs">Load saved graphs</button>
        <button id="load-selected-graph">Load selected graph</button>
        <button id="load-graph-history">Load graph history</button>
      </div>

      <div class="database-grid">
        <div class="field">
          <label for="saved-map-select">Saved maps</label>
          <select id="saved-map-select">
            <option value="">No saved maps loaded</option>
          </select>
        </div>
        <div class="field">
          <label for="experiment-select">Experiments</label>
      ...
      <div class="status" id="database-status-text">Database status: graph not saved yet.</div>
      <div class="status info" id="saved-map-summary">No saved map selected.</div>
    </section>
    ...
    <section class="panel comparison-panel">
      <h2>Saved graph history</h2>
      <div class="comparison-empty" id="history-empty">
        Load a saved graph and select an experiment to view stored algorithm history.
      </div>
  ...
  <script>
    ...
    const MAPS_URL = "http://127.0.0.1:8000/maps";
    const CELL_SIZE = 48;
    const MARGIN = 56;
    ...
    const savedMapSummary = document.getElementById("saved-map-summary");
    ...
    const saveResultsToggle = document.getElementById("save-results-toggle");
    const mapNameInput = document.getElementById("map-name");
    const mapSlugInput = document.getElementById("map-slug");
    ...
    function mappingPairsToLookup(pairs) {{
      const mapping = {{}};
      for (const pair of pairs || []) {{
        mapping[`${{pair.row}},${{pair.col}}`] = pair.node_id;
      }}
      return mapping;
    }}
    ...
    function setDatabaseStatus(message, tone = "info") {{
      databaseStatusText.textContent = message;
      databaseStatusText.classList.remove("info", "active", "error");
      databaseStatusText.classList.add(tone);
    }}

    function updateSavedMapSummary(message = null) {{
      if (message) {{
        savedMapSummary.textContent = message;
        return;
      }}

      if (!state.savedMapId) {{
        savedMapSummary.textContent = "No saved map selected.";
        return;
      }}

      savedMapSummary.textContent = `Saved map: ${{savedMapSelect.options[savedMapSelect.selectedIndex]?.text || state.savedMapId}} | id ${{state.savedMapId}}`;
    }}

    function clearHistoryResults(message = "Load a saved graph and select an experiment to view stored algorithm history.") {{
    ...
    function renderHistoryResults(results) {{
      ...
      for (const result of results) {{
        const row = document.createElement("tr");
        row.innerHTML = `
          <td>${{formatLabel(result.algorithm)}}</td>
          <td>${{formatLabel(result.status)}}</td>
          <td>${{formatCost(result.total_cost)}}</td>
          <td>${{result.runtime_ms ?? "N/A"}}</td>
          <td>${{result.cost_rank ?? "N/A"}}</td>
          <td>${{result.speed_rank ?? "N/A"}}</td>
          <td>${{formatLabel(result.reliability_label)}}</td>
          <td>${{formatLabel(result.speed_label)}}</td>
        `;
    ...
    function markGraphUnsaved(reason = "Current graph has unsaved changes.") {{
      state.savedMapId = null;
      state.savedMapPointToNodeId = {{}};
      saveResultsToggle.checked = false;
      updateSavedMapSummary(reason);
      setDatabaseStatus("Database status: current graph is not saved.", "info");
    }}
    ...
    function trafficHasWeightedValues(traffic) {{
      if (!traffic) {{
        return false;
      }}
      for (const direction of ["north", "south", "east", "west"]) {{
        for (const row of traffic[direction] || []) {{
    ...
    function chooseDefaultEndpoints() {{
      ...
      traversable.sort((a, b) => (a.col - b.col) || (a.row - b.row));
      state.start = traversable[0];
      state.goal = traversable[traversable.length - 1];
    }}

    async function loadSavedMaps() {{
      setDatabaseStatus("Loading saved maps from Supabase...", "active");
      try {{
        const response = await fetch(MAPS_URL);
        const payload = await response.json();
        if (!response.ok) {{
          throw new Error(payload.detail || "Could not load saved maps.");
        }}

        state.savedMaps = payload.maps || [];
        savedMapSelect.innerHTML = "";
        if (state.savedMaps.length === 0) {{
          savedMapSelect.innerHTML = '<option value="">No saved maps found</option>';
          updateSavedMapSummary("No saved maps found yet.");
        }} else {{
          for (const map of state.savedMaps) {{
            const option = document.createElement("option");
            option.value = map.id;
            option.textContent = `${{map.name}} (${{map.slug}})`;
            savedMapSelect.appendChild(option);
          }}
          if (state.savedMapId) {{
            savedMapSelect.value = state.savedMapId;
          }}
          updateSavedMapSummary("Saved maps loaded. Choose one to load its graph or history.");
        }}
        setDatabaseStatus("Saved maps loaded from Supabase.", "active");
      }} catch (error) {{
        setDatabaseStatus(`Could not load saved maps: ${{error.message}}`, "error");
      }}
    }}

    async function saveCurrentGraph() {{
      const name = mapNameInput.value.trim() || "Generated Manhattan Map";
      const slug = mapSlugInput.value.trim() || slugify(name) || `generated-map-${{Date.now()}}`;
      mapNameInput.value = name;
      mapSlugInput.value = slug;

      setDatabaseStatus("Saving current graph to Supabase...", "active");
      try {{
        const response = await fetch(MAPS_URL, {{
          method: "POST",
          headers: {{
          ...
          body: JSON.stringify({{
            name,
            slug,
            description: mapDescriptionInput.value.trim() || null,
            grid: state.grid,
            traffic: state.trafficEnabled ? state.traffic : null,
            stoplights: serializeStoplights(),
            metadata: {{
              generated_from_demo: true,
              traffic_enabled: state.trafficEnabled,
              stoplight_count: serializeStoplights().length,
            }},
          }}),
        }});
        const payload = await response.json();
        if (!response.ok) {{
          throw new Error(payload.detail || "Could not save graph.");
        }}

        state.savedMapId = payload.map_id;
        state.savedMapPointToNodeId = mappingPairsToLookup(payload.point_node_pairs);
        savedMapSelect.innerHTML = `<option value="${{payload.map_id}}">${{payload.map?.name || name}} (${{payload.map?.slug || slug}})</option>`;
        savedMapSelect.value = payload.map_id;
        updateSavedMapSummary(`Saved graph '${{payload.map?.name || name}}' with ${{payload.node_count}} nodes and ${{payload.edge_count}} directed edges.`);
        setDatabaseStatus("Graph saved to Supabase.", "active");
        await loadSavedMaps();
        await loadMapExperiments(payload.map_id);
      }} catch (error) {{
        setDatabaseStatus(`Could not save graph: ${{error.message}}`, "error");
      }}
    }}

    async function loadSelectedMapGraph() {{
      const mapId = savedMapSelect.value;
      if (!mapId) {{
        setDatabaseStatus("Choose a saved map before loading it.", "error");
        return;
      }}

      setDatabaseStatus("Loading saved graph from Supabase...", "active");
      try {{
        const response = await fetch(`${{MAPS_URL}}/${{mapId}}`);
        const payload = await response.json();
        if (!response.ok) {{
          throw new Error(payload.detail || "Could not load the saved graph.");
        }}

        state.grid = payload.grid || [[]];
        state.traffic = payload.traffic;
        state.trafficEnabled = trafficHasWeightedValues(payload.traffic);
        state.stoplights = {{}};
        for (const signal of payload.traffic_signals || []) {{
          if (signal.row === null || signal.col === null || signal.row === undefined || signal.col === undefined) {{
            continue;
          }}
          state.stoplights[`${{signal.row}},${{signal.col}}`] = {{
            row: signal.row,
            col: signal.col,
            average_wait_seconds: signal.average_wait_seconds,
            light_cycle_seconds: signal.light_cycle_seconds,
            has_stoplight: true,
            metadata: signal.metadata || {{}},
          }};
        }}
        state.savedMapId = mapId;
        state.savedMapPointToNodeId = mappingPairsToLookup(payload.point_node_pairs || []);
        mapNameInput.value = payload.map?.name || mapNameInput.value;
        mapSlugInput.value = payload.map?.slug || mapSlugInput.value;
        mapDescriptionInput.value = payload.map?.description || "";
        chooseDefaultEndpoints();
        clearComparisonResults("Loaded a saved graph. Run algorithms again to compare fresh results.");
        clearHistoryResults("Saved experiment history loaded below when available.");
        findPathInBrowser();
        updateSavedMapSummary(`Loaded saved map '${{payload.map?.name || mapId}}'.`);
        setDatabaseStatus("Saved graph loaded from Supabase.", "active");
        await loadMapExperiments(mapId);
      }} catch (error) {{
        setDatabaseStatus(`Could not load saved graph: ${{error.message}}`, "error");
      }}
    }}

    async function loadMapExperiments(mapId = savedMapSelect.value) {{
      if (!mapId) {{
        clearHistoryResults("Choose a saved graph before loading history.");
        return;
      }}

      try {{
        const response = await fetch(`${{MAPS_URL}}/${{mapId}}/experiments`);
        const payload = await response.json();
        if (!response.ok) {{
          throw new Error(payload.detail || "Could not load experiment history.");
        }}

        state.experiments = payload.experiments || [];
        experimentSelect.innerHTML = "";
        if (state.experiments.length === 0) {{
          experimentSelect.innerHTML = '<option value="">No experiments saved yet</option>';
          clearHistoryResults("This saved graph has no experiment history yet.");
          return;
        }}
        ...
        for (const experiment of state.experiments) {{
          ...
          option.textContent = `${{experiment.id}} | traffic ${{experiment.traffic_enabled ? "on" : "off"}} | stoplights ${{experiment.stoplights_enabled ? "on" : "off"}}`;
          experimentSelect.appendChild(option);
        }}
        await loadSelectedExperimentResults();
      }} catch (error) {{
        clearHistoryResults(`Could not load graph history: ${{error.message}}`);
      }}
    }}
    ...
    async function loadSelectedExperimentResults() {{
      ...
      if (!experimentId) {{
        clearHistoryResults("Choose an experiment to view saved algorithm history.");
        return;
      }}

      try {{
        const response = await fetch(`http://127.0.0.1:8000/experiments/${{experimentId}}/results`);
        const payload = await response.json();
        if (!response.ok) {{
          throw new Error(payload.detail || "Could not load experiment results.");
        }}
        const results = payload.results || [];
        if (results.length === 0) {{
          clearHistoryResults("This experiment has no stored comparison rows.");
          return;
        }}
        renderHistoryResults(results);
      }} catch (error) {{
        clearHistoryResults(`Could not load experiment results: ${{error.message}}`);
      }}
    }}
    ...
    function toggleCell(row, col) {{
      ...
      markGraphUnsaved("Current graph has unsaved roadblock changes.");
      clearComparisonResults("Grid changed. Run the backend comparison again to refresh the table.");
      findPathInBrowser();
    }}
    ...
    function toggleStoplight(row, col) {{
      ...
      markGraphUnsaved("Current graph has unsaved stoplight changes.");
    ...
    async function runBackendPathfinding() {{
      ...
      if (saveResultsToggle.checked) {{
        if (!state.savedMapId) {{
          setApiStatus("Save the graph to the database before saving algorithm results.", "error");
          return;
        }}

        const startNodeId = state.savedMapPointToNodeId[pointKey(state.start)];
        const endNodeId = state.savedMapPointToNodeId[pointKey(state.goal)];
        if (!startNodeId || !endNodeId) {{
          setApiStatus("The selected start or goal point does not exist in the saved map nodes. Reload or re-save the graph first.", "error");
          return;
        }}

        requestBody.save = true;
        requestBody.map_id = state.savedMapId;
        requestBody.start_node_id = startNodeId;
        requestBody.end_node_id = endNodeId;
      }}
      ...
      try {{
        ...
        if (payload.experiment_id) {{
          setDatabaseStatus(`Saved experiment ${{payload.experiment_id}} and ${{Object.keys(payload.saved_run_ids || {{}}).length}} algorithm runs.`, "active");
          await loadMapExperiments(state.savedMapId);
        }}
      }} catch (error) {{
    ...
    mapNameInput.addEventListener("input", () => {{
      if (!mapSlugInput.dataset.userEdited) {{
        mapSlugInput.value = slugify(mapNameInput.value);
      }}
    }});

    mapSlugInput.addEventListener("input", () => {{
      mapSlugInput.dataset.userEdited = "true";
    }});
    ...
    document.getElementById("save-graph").addEventListener("click", saveCurrentGraph);
    document.getElementById("load-saved-graphs").addEventListener("click", loadSavedMaps);
    document.getElementById("load-selected-graph").addEventListener("click", loadSelectedMapGraph);
    document.getElementById("load-graph-history").addEventListener("click", () => loadMapExperiments(savedMapSelect.value));
    savedMapSelect.addEventListener("change", () => {{
      state.savedMapId = savedMapSelect.value || null;
      updateSavedMapSummary(
        state.savedMapId
          ? "Saved map selected. Load the graph or history when ready."
          : "No saved map selected."
      );
      clearHistoryResults();
    }});
    experimentSelect.addEventListener("change", loadSelectedExperimentResults);
    ...
    document.getElementById("generate-grid").addEventListener("click", () => {{
      ...
      if (success) {{
        markGraphUnsaved("Generated a new unsaved graph.");
    ...
    trafficButton.addEventListener("click", () => {{
      ...
      markGraphUnsaved("Current graph has unsaved traffic changes.");
    ...
    if (!generateRandomGrid(Number(rowsInput.value), Number(colsInput.value), Number(densityInput.value) / 100)) {{
      ...
      markGraphUnsaved("Generated a fresh graph that is not yet saved.");
